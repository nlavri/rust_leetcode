...
impl Solution {
    ...
    fn merge_rec(tail: &mut ListNode, list1: &Option<Box<ListNode>>, list2: &Option<Box<ListNode>>) {
        match (list1, list2) {
            (None, None) => return,
            (None, Some(l2)) => {
                Solution::append(tail, l2.val);
                Solution::merge_rec(&mut *tail.next.as_mut().unwrap(), list1, &list2.as_ref().unwrap().next);
            },
            (Some(l1), None) => {
                Solution::append(tail, l1.val);
                Solution::merge_rec(&mut *tail.next.as_mut().unwrap(), &list1.as_ref().unwrap().next, list2);
            },
            (Some(l1), Some(l2)) => {
                if l1.val <= l2.val {
                    Solution::append(tail, l1.val);
    ...
    fn merge_internal2(list1: &Option<Box<ListNode>>, list2: &Option<Box<ListNode>>) -> Option<Box<ListNode>> {
        ...
        Solution::merge_rec(&mut head, l1_ptr, l2_ptr);

        return Some(Box::new(head));

    }

    fn merge_internal(list1: &Option<Box<ListNode>>, list2: &Option<Box<ListNode>>) -> Option<Box<ListNode>> {
    ...
    pub fn merge_two_lists(list1: Option<Box<ListNode>>, list2: Option<Box<ListNode>>) -> Option<Box<ListNode>> {
        match (&list1, &list2) {
            (Some(l1), Some(l2)) => Solution::merge_internal(&list1, &list2),
